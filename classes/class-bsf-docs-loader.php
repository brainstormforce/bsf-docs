...
if ( !class_exists( 'Bsf_Doc_Loader' ) ) {
	...
	final class Bsf_Doc_Loader {
	    ...
	    {		
	    	...
	    	if( $is_single_template_on == '1' || $is_single_template_on === false ) {
	    	...
	    	if( $is_cat_template_on == '1' || $is_cat_template_on === false ) {
	    ...
	    function register_options_menu() {
    		...
    		add_submenu_page(
				...
				__( "Settings", "documentation-wordpress" ),
				__( "Settings", "documentation-wordpress" ),
				"manage_options",
	    ...
	    function render_options_page() {
	    	require_once BSF_DOCS_BASE_DIR . 'includes/options-page.php';
	    }
		...
		function php_version_notice() {
			...
			$message = __( 'Your server seems to be running outdated, unsupported and vulnerable version of PHP. You are advised to contact your host provider and upgrade to PHP version 5.6 or greater.', 'documentation-wordpress' ); 
		...
		static private function load_files() {
			...
			require_once BSF_DOCS_BASE_DIR . 'includes/bsf-docs-widget.php';
		}
		...
		function enqueue_front_scripts() {
			...
			if( $is_live_search == '1' || $is_live_search === false ) {
				...
				wp_localize_script( 'bsf-searchbox-script', 'bsf_ajax_url', 
					array( 
