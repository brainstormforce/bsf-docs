...
add_shortcode( 'wp_docs_live_search', 'bsf_doc_render_search_box' );
add_shortcode( 'wp_docs_category_list', 'bsf_render_category_list' );
...
function bsf_doc_render_search_box( $atts, $content = null ) {
	...
	extract( shortcode_atts( array(
		'placeholder' => __( "Enter search string", "documentation-wordpress" )
	), $atts ));
	    ...
	    <div class="bsf-search-container">
	    	<div id="bsf-search-wrap">
				<form role="search" method="get" id="bsf-searchform" class="clearfix" action="<?php echo home_url(); ?>">
					<input type="text" placeholder="<?php echo esc_attr( $placeholder ); ?>" onfocus="if (this.value == '') {this.value = '';}" onblur="if (this.value == '')  {this.value = '';}" value="" name="s" id="bsf-sq" autocapitalize="off" autocorrect="off" autocomplete="off">
					...
					<button type="submit" id="bsf-searchsubmit">
					    ...
					    <span><?php _e( "Search", "documentation-wordpress" ); ?></span>
					</button>
				</form>
	      </div>
	    </div>
...
function bsf_render_category_list( $atts, $content = null ) {
	...
	$taxonomy_objects = get_terms( $category, array( "hide_empty" => false ) );

	?>

	<?php $doc_title = get_option( 'bsf_doc_title' ); 
	...
	<div class="bsf-categories-wrap clearfix">
		...
		<?php foreach ( $taxonomy_objects as $key => $object ) {
			...
			<div class="bsf-cat-col" >
				<a class="bsf-cat-link" href="<?php echo esc_url( $cat_link ); ?>">
					...
					<span class="bsf-cat-count">
						<?php echo $count . " " . __( "Articles", "documentation-wordpress" ) ; ?> 
					</span>
				</a>
			</div>
